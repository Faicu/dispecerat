...
// replace the operators section
const opRegex = /\{gameState\.operators\.length > 1 && \([\s\S]*?<\div>\s*<\/div>\s*\)\}/;
code = code.replace(opRegex, `{gameState.operators.length > 0 && (
...
// replace the rentedOperators section
const aiRegex = /\{gameState\.rentedOperators && gameState\.rentedOperators\.length > 0 && \([\s\S]*?<\div>\s*<\/div>\s*\)\}/;
code = code.replace(aiRegex, `{gameState.rentedOperators && gameState.rentedOperators.length > 0 && (
